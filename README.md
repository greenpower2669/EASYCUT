...
Android SDK 36, JDK 17, Gradle 8.13, Kotlin/Compose, Media3 et FFmpegKit ; fichiers sous `app/`. Identifiant d'installation `com.fabvidedit.app.poc` et packages internes volontairement conservés pour ne pas réécrire le format et le stockage existant. Nom visible **EASYCUT**, versionName `0.0.1`, versionCode `39`. Nouveaux exports destinés à `Films/EASYCUT`.

```bash
gradle :app:testDebugUnitTest :app:lintDebug :app:assembleDebug :app:bundleDebug
```

Noms de compilation : `EASYCUT-v0.0.6-arm64-debug.apk` et `EASYCUT-v0.0.6-arm64-debug.aab`. Une GitHub prerelease est publiée automatiquement après réussite de la CI sur `main` (ou lancement manuel avec `publish=true`) ; elle ne signifie pas validation sur téléphone. Les APK debug signés sur un autre appareil de build peuvent ne pas s'installer par-dessus l'ancien éditeur : sauvegarder les projets avant toute désinstallation.
...
## Test de cadrage V1 et placement v0.0.6
...
## Échelle souhaitée et export — v0.0.6
Le zoom utilisateur à 321 % ou 380 % est volontaire et conservé dans les keyframes. Le lecteur de secours ajuste maintenant la source au canevas en mode FIT puis applique le zoom/pan/rotation via `TextureView.setTransform`, au lieu de transformer tout l'AndroidView dans un cadre potentiellement distordu. L'export Media3 reste inchangé : les tests automatiques contrôlent la géométrie, et la concordance GPU du MP4/aperçu doit encore être confirmée sur téléphone avec le média d'origine. Aucun « reset » du zoom n'est effectué.


## v0.0.6 — gestes et aperçu (candidat)
Le zoom/pan à deux doigts agit immédiatement sur la dernière texture vidéo décodée ; une animation de keyframes sur le mode simplifié actualise sa matrice hors des recompositions complètes de la timeline. Les modifications visuelles ne doivent plus réinitialiser lecteur et audio. L'export Media3, les keyframes, l'identité Android et les projets restent inchangés. Compteurs `PERF preview` uniquement dans le journal local, sans média/URI. Un véritable proxy basse résolution n'est PAS créé : il reste une amélioration ultérieure pour les sources très difficiles à décoder. Tester la concordance MP4/aperçu et le pinch sur téléphone.
