...
### FAB-IMPORT-UNIFIED-012 — Importation unifiée et split facultatif (demande Fab, 25/09/2026 ; code pilote v0.0.17)
- **Intention de Fab à préserver :** au départ EasyCut devait faciliter les objets de montage **groupés / composites**, éventuellement imbriqués : un même ensemble peut contenir vidéo + son, images, deuxième vidéo et d'autres médias. Déplacement collectif facile et synchronisé dans la timeline, sans perdre la possibilité d'éditer ou détacher chaque composant. Ne PAS confondre cette liaison logique avec la séparation physique des flux.
- **Comportement demandé pour une future réalisation :** à l'import, privilégier **un seul fichier/conteneur audiovisuel** conservant ses pistes et métadonnées, ainsi qu'un objet lié vidéo+son. Afficher une **case à cocher « Désactiver le split (test) » DÉCOCHÉE par défaut** : décochée = ancien parcours de séparation vidéo/audio ; cochée = parcours source audiovisuelle unifiée et, si pertinent, des flux multiples ; décrire clairement le coût en temps et en stockage. Étudier aussi l'extraction/détachement audio APRÈS import, sur demande : création d'une piste audio éditable sans doubler le son, copie physique uniquement si nécessaire. Ne pas présumer que CapCut implémente un mécanisme précis en interne : son interface est l'inspiration, pas une preuve d'architecture.
- **Diagnostic obligatoire avant de coder :** cartographier URI/clonage → premier inventaire FFprobe → validation des streams → scan éventuel → StreamSourceMaterializer.split → stream-copy/remux et lecteur. Une erreur FFprobe au **premier inventaire** se produit AVANT split, donc ne doit pas lui être attribuée ; tester distinctement les erreurs de scan, séparation, muxer, décodeur, et les pertes de métadonnées/horodatages éventuelles. Ne pas annoncer que l'import unique contourne FFprobe si l'inventaire reste obligatoire. Comparer sur le MÊME média import unique/split lorsque cela permettra d'isoler la cause ; les deux vidéos déjà signalées par Fab étaient distinctes.
- **Architecture à auditer :** possibilité de lecture/édition native de la source A/V unique dans Media3 ; relation objets groupés ↔ fichier source ↔ flux internes ; prise en charge audio intégré, pistes multiples, alignement et actions collectives/détachées ; conservation non destructive des sources. Si la lecture directe échoue, offrir un repli diagnostiqué et explicite, notamment split, sans remplacement silencieux, sans inventer une compatibilité universelle. Préserver le traitement nécessaire des vrais changements de résolution et les chemins historiques d'export/multipiste.
- **Critères de validation ultérieurs :** vidéo+son ordinaire importée sans extraction physique systématique ; case split fonctionnelle et non cochée par défaut ; groupement vidéo/audio/images/autres objets et déplacement collectif sans dérive temporelle ; extraction/détachement volontaire sans audio doublé ; ancien projet et mode split préservés ; comparaison fichiers, stockage, durée d'import, métadonnées et rendu/export sur fichiers réels. Étude et tests Android avant toute déclaration de correction FFprobe.
- **Statut actuel : pilote v0.0.17 codé sur branche de travail ; tests CI, livraison et validation Android non attestés. Le groupement composite libre d'images, vidéos et sons reste une extension non codée.** Pixels non carrés v0.0.16 : amélioration confirmée par Fab, dossier en vigilance uniquement et HORS périmètre de cette mission tant qu'aucun bug nouveau n'est observé.
...
## Précision de Fab — 25/09/2026 : source unifiée, objets groupés, split sur demande
FAB-IMPORT-UNIFIED-012 est ajouté aux missions ouvertes, sans réouvrir les validations ni remplacer FAB-SOURCE-COMPAT-011. Il décrit le besoin d'origine de groupement multi-objets ; précision ultérieure : la case « Désactiver le split (test) » est décochée par défaut, donc le split historique reste actif. Pas de code dans cette intervention documentaire.

## Correction explicite de Fab et lancement du pilote v0.0.17 — 25/09/2026
