...
### FAB-IMPORT-UNIFIED-012 — Importation unifiée et split facultatif (demande Fab, 25/09/2026 ; CODE PAS)
...
- **Comportement demandé pour une future réalisation :** à l'import, privilégier **un seul fichier/conteneur audiovisuel** conservant ses pistes et métadonnées, ainsi qu'un objet lié vidéo+son. Afficher une **case à cocher « Séparer les pistes à l'import (split) » désactivée par défaut**. Si cochée, permettre le mode historique de séparation vidéo/audio et, si pertinent, des flux multiples ; décrire clairement le coût en temps et en stockage. Étudier aussi l'extraction/détachement audio APRÈS import, sur demande : création d'une piste audio éditable sans doubler le son, copie physique uniquement si nécessaire. Ne pas présumer que CapCut implémente un mécanisme précis en interne : son interface est l'inspiration, pas une preuve d'architecture.
...
- **Statut : ORDRE DOCUMENTÉ, CONCEPTION/DIAGNOSTIC SEULEMENT ; AUCUN CODE, APK/AAB OU RELEASE AUTORISÉ PAR CETTE DEMANDE.** Pixels non carrés v0.0.16 : amélioration confirmée par Fab, dossier en vigilance uniquement et HORS périmètre de cette mission tant qu'aucun bug nouveau n'est observé.
...
FAB-IMPORT-UNIFIED-012 est ajouté aux missions ouvertes, sans réouvrir les validations ni remplacer FAB-SOURCE-COMPAT-011. Il décrit le besoin d'origine de groupement multi-objets et la case split décochée par défaut. Pas de code dans cette intervention documentaire.
